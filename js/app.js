(function () {
    ...
    function calcDayStats(profile, dayData) {
        ...
        let totalEaten = 0; MEAL_TYPES.forEach(t => totalEaten += calcFoodSum(dayData.meals?.[t]));
        const remaining = allowance - totalEaten;
        const actualDeficit = bmr * BMR_MULTIPLIER + totalExercise - totalEaten;
    ...
    function calcDayNutrition(dayData) {
        let p = 0, c = 0;
        MEAL_TYPES.forEach(t => (dayData.meals[t] || []).forEach(item => { const n = calcFoodNutrition(item); p += n.protein; c += n.carbs; }));
    ...
    function switchPage(pageName) {
        ...
        document.getElementById('page' + capFirst(pageName)).classList.add('active');
    ...
    function renderHome() {
        ...
        MEAL_TYPES.forEach(type => renderMealList(type, todayData.meals[type], false));
        MEAL_TYPES.forEach(type => {
            const el = document.getElementById('sum' + capFirst(type));
            if (el) el.textContent = '小计 ' + Math.round(calcFoodSum(todayData.meals[type])) + ' kcal';
        });
    ...
    function setupEventDelegation() {
        // ✅ 关键修复：handler 接收实际点击的元素 (el)，而不是原始事件 e
        // 因为 e.currentTarget 指向 document.body，无法读取按钮的 data-*
        const ACTION_MAP = {
        ...
        document.body.addEventListener('click', (e) => {
            ...
            if (!el) return;
            // 只在处理非 submit 类型时才 preventDefault，避免误伤表单
    ...
    document.addEventListener('DOMContentLoaded', () => {
        ...
        MEAL_TYPES.forEach(t => historyMealsContainer.appendChild(buildMealSection(t, true)));
